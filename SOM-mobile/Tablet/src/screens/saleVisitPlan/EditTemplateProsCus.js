...
import { ALEART_WARNING_WORD } from '../../utility/enum';
...
const EditTemplateProsCus = (props) => {
    ...
    const [formScreenView, setformScreenView] = useState(props.route.params.formScreenView);
    ...
    const [dataFromEditPlan, setDataFromEditPlan] = useState([]);
    ...
    useEffect(() => {
        if (saleVisitPlanReducer.taskSpecialCreateplan) {
            ...
            if (dataProsCusLoc.custCode == "") {
                if (!saleVisitPlanReducer?.taskSpecialCreateplan) return
            ...
            if (dataProsCusLoc.listTask) {
                ...
                if (!saleVisitPlanReducer?.taskTemplatCreateplan) return
    ...
    const handleSaveTask = (item) => {
        ...
        if (taskVisitPlanList && taskVisitPlanList.length != 0) {
            ...
            newDataAdd = saleVisitPlanReducer.updateValueList.map((item) => {
                if (item.prospAccId == newTaskAdd.prospAccId && item.prospId == newTaskAdd.prospId) {
                    ...
                    return {
                        ...item, 
                        listTask: newTaskAdd.listTask ? newTaskAdd.listTask : [], 
        ...
        if (timeStart && timeEnd && timeStart > timeEnd) {
            ...
            if (newDataAdd.length == 0) {
                ...
                navigation.navigate('CreatePlanForMeScreen');
            }
        }  
    };
    ...
    return (
        <View style={{flex: 1, backgroundColor: colors.white, padding: 20, paddingTop:'5%'}}>
            <Header />
            {
                dataProsCusLoc && dataProsCusLoc.prospId ? 
                <View>
                    <View style={[styles.SearchArea,styles.styleShadow]}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20, marginTop: '2%', marginLeft: -5}}>
                            <View style={{ flexDirection: 'row' }}>
                                <RadioButton
                            ...
                            <View style={{width:350, paddingRight:10}}>
                                {
                                    redioSelectType === 'T' && <SelectDropdown
                                        ...
                                        NotFillter={true}
                                        REQUIRETITLE
                                        massageError={language.SELECTTASK}
                                ...
                                }
                                {
                                    redioSelectType === 'S' && <SelectDropdown
                                        ...
                                        NotFillter={true}
                                        REQUIRETITLE
                                        massageError={language.SELECTTASKSPECIAL}
                                        onPress={(item)=> handleAddTaskNew(item)}
                                    />
                                }
                            </View>
                ...
                <View>
                    <View style={[styles.SearchArea,styles.styleShadow]}>
                        ...
                        <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                            <View style={{flexDirection: 'row'}}>
                                <View style={{borderBottomColor: colors.gray, borderBottomWidth: 1, marginStart: 15}}>
                                    <Text numberOfLines={1}>{dataProsCusLoc.locNameTh ? dataProsCusLoc.locNameTh : dataProsCusLoc.accName}</Text>
                                </View>
                            </View>
                        </View>
                        <View style={{marginTop: '2%', flexDirection: 'row'}}>
...
export default EditTemplateProsCus;