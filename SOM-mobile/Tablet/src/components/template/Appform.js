...
function Appform({ route, navigation }) {
    ...
    const renderList = ({item,index}) =>{
        ...
        if(item.prerequistOrderNo != ""){
            ...
            // if(forms[parseInt(item.prerequistOrderNo)-1] && forms[parseInt(item.prerequistOrderNo)-1].ansVal[0] && !forms[parseInt(item.prerequistOrderNo)-1].ansVal[0].val) disable =false
        }
        if(ansType === '1'){
        ...
        if(ansType === '9'){
            ...
            {/* <PickerDate dateValue={item.ansVal[0].val? dayjs(item.ansVal[0].val).format('YYYY-MM-DD') : null} onChange={(date)=>{
                let obj = forms;
                obj[index].ansVal[0].val = dayjs(date).format('YYYYMMDD')
                setForms(obj)
                validate()
    ...
    return (
        <View  style={{backgroundColor:'white'}}>
             ...
             <ModalWarning
                ...
                />
             <ScrollView keyboardShouldPersistTaps='always' style={{padding:20,marginBottom:100,backgroundColor:'white'}}>
                <FlatList 
                        data={forms}
