  ...
  let options = {
    ...
    const renderList = ({item,index}) =>{
       ...
       if(image){
        return (
          <View >
              <Image source={{uri:convertImage(image)}} style={{width:width*0.45,height:height*0.3,borderRadius:10,borderWidth:0.2,marginTop:20}} resizeMode={'stretch'} />
           <TouchableOpacity style={{position:'absolute',left:width*0.4,top:30}} onPress={()=>{
               let lists = [...list]
               if(lists.length==1){
    ...
    <FlatList
     ...
     ListHeaderComponent={()=>{
         return <TouchableOpacity disabled={!props.disable} style={{marginLeft:20,marginBottom:20,marginTop:20}} onPress={()=> addImage()}>
                ...
                <Image source={require('../../assets/images/ic_choose_camera.png')} style={{width:60,height:60}}/>
             </TouchableOpacity>
     }}
