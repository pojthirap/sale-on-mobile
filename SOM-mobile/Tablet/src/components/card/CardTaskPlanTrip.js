        ...
        return (
            <View style={styles.container}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    {/* {CardArea} */}
                    <View style={{ backgroundColor: colors.greenPlanTripCard, height: 110, width: 430, borderRadius: 25 }}>
                        <View style={{ flexDirection: 'row', paddingHorizontal: 15, alignItems: 'center', paddingTop: 30 }}>
                            <View>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <View style={{ paddingRight: 10 }}>
                                <TouchableOpacity disabled={!isStart || isComplete} onPress={() => { validateRaduis(TaskPlanTripData.item), setMile(TaskPlanTripData.item.visitCheckinMileNo ? `${TaskPlanTripData.item.visitCheckinMileNo}` : '') }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Icon type="Ionicons" name="flag-outline" style={{ fontSize: 20, paddingRight: 5, color: !TaskPlanTripData.item.visitCheckinDtm ? colors.grayButton : colors.primary }} />
                                        <Text style={TaskPlanTripData.item.visitCheckinDtm ? { fontSize: 22, color: colors.primary } : { fontSize: 22 }}>Check in</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                            <View style={{ paddingRight: 10 }}>
                                {!isStart || !TaskPlanTripData.item.visitCheckinDtm || (TaskPlanTripData.item.reasonNotVisitId && !TaskPlanTripData.item.visitCheckoutDtm) ? 
                                    <View disabled={!isStart || isComplete || !TaskPlanTripData.item.visitCheckinDtm}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Icon type="Ionicons" name="log-out-outline" style={{ fontSize: 20, paddingRight: 5, color: TaskPlanTripData.item.visitCheckoutDtm ? colors.grayButton : colors.grayButton }} />
                                            <Text style={{ fontSize: 22, color: TaskPlanTripData.item.visitCheckoutDtm ? colors.grayButton : colors.grayButton }}>Check out</Text>
                                        </View>
                                    </View> 
                                    : 
                                    <TouchableOpacity disabled={!isStart || isComplete || !TaskPlanTripData.item.visitCheckinDtm} onPress={() => onPressForCheckOut(true, TaskPlanTripData.item)}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <Icon type="Ionicons" name="log-out-outline" style={{ fontSize: 20, paddingRight: 5, color: TaskPlanTripData.item.visitCheckoutDtm ? colors.primary : colors.black }} />
                                            <Text style={{ fontSize: 22, color: TaskPlanTripData.item.visitCheckoutDtm ? colors.primary : colors.black }}>Check out</Text>
                                        </View>
                                    </TouchableOpacity>
                                }
                            ...
                            <View>
                                <TouchableOpacity disabled={!isStart || TaskPlanTripData.item.visitCheckoutDtm || TaskPlanTripData.item.reasonNotVisitId} onPress={() => reasonNo(TaskPlanTripData.item)}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Icon type="MaterialCommunityIcons" name="close-box-outline" style={{ fontSize: 20, color: TaskPlanTripData.item.reasonNotVisitId ? colors.primary : 'red', paddingRight: 5 }} />
                                        <Text style={{ color: TaskPlanTripData.item.reasonNotVisitId ? colors.primary : 'red', fontSize: 22 }}>ไม่เข้าสถานที่</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                        </View>
                            ...
                            TaskPlanTripData.item.adhocFlag == 'Y' ?
                                <View style={{ alignItems: 'center', marginTop: '6%' }}>
                                    <TouchableOpacity disabled={ isComplete || TaskPlanTripData.item.visitCheckinDtm || TaskPlanTripData.item.visitCheckoutDtm || TaskPlanTripData.item.reasonNotVisitId } onPress={() => onPressConfirmDelete(TaskPlanTripData.item, true)}>
                                        <View style={{ borderWidth: 2, paddingHorizontal: '10%', paddingVertical: '1%', borderColor: isComplete || TaskPlanTripData.item.visitCheckinDtm || TaskPlanTripData.item.visitCheckoutDtm || TaskPlanTripData.item.reasonNotVisitId ? colors.grayButton : colors.redButton, borderRadius: 10 }}>
                                            <Text style={{ color: isComplete || TaskPlanTripData.item.visitCheckinDtm || TaskPlanTripData.item.visitCheckoutDtm || TaskPlanTripData.item.reasonNotVisitId ? colors.black : colors.redButton, fontSize: FONT_SIZE.TEXT }}>Delete</Text>
                                        </View>
                                    </TouchableOpacity>
                                </View>
                            : null
                ...
                <View style={{ margin: '5%' }}>
                    ...
                    <View style={{ marginTop: '3%', width: '100%' }}>
                        <CTextInput
                            ...
                            { fontSize: FONT_SIZE.TEXT, height: 50, fontFamily: 'THSarabunNew', color: colors.black, width: '100%' }]}
                            onChangeText={(e) => {
                    ...
                    <View style={{ marginTop: '3%', width: '100%' }}>
                        <CTextInput
                            ...
                            style={[styles.inputbox, { backgroundColor: colors.white },
                            { fontSize: FONT_SIZE.TEXT, height: 120, fontFamily: 'THSarabunNew', color: colors.black, width: '100%' }]}
                            onChangeText={(e) => {
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 20 }}>
                        ...
                        <View style={{ alignSelf: 'center' }}>
                            <TextInput
                                ...
                                style={{ fontFamily: 'THSarabunNew', fontSize: 30, width: 120 }}
                                value={mile}
                                onChangeText={(e) => {
                                    if (!e) return setMile('')
                                    if (/^\d+$/.test(e))
