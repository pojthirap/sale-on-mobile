    ...
    const Card = (items) =>{
        return(
            <View style={styles.container, {justifyContent: 'center'}}>
                ...
                style={{width: isAddress ? 380 : '100%', borderRadius: 30, minHeight: isAddress || isSaleData ? 200 : 180}}
            />
            <View style={[styles.cardArea, {marginVertical: '2%', width: '100%'}]}>
                <View style={{alignItems:'flex-start',marginVertical:'2%',marginHorizontal:'7%'}}>
                    {isAddress ?
                        <View style={{paddingHorizontal:'5%',paddingTop:5}}>
                            <Text style={{color:colors.white,fontSize:25, fontWeight:'bold'}} numberOfLines={2}>{items.item.addressFullName}</Text> 
                        </View>
                    :
                    isSaleData ?
                        <View style={{paddingHorizontal:'5%', marginTop:5}}>
                            <Text style={{color:colors.white,fontSize:28, fontWeight:'bold'}}>{items.item.orgNameTh}</Text> 
                        </View>
                    :
                    isContect ?
                        // <View style={{flexDirection: 'row'}}>
                        //     <View style={{paddingHorizontal:'2%', marginTop:15, width: "50%"}}>
                        //         <Text style={{color:colors.white,fontSize:30, fontWeight:'bold'}} numberOfLines={1}>{items.item.firstName}</Text> 
                        //     </View>
                        //     <View style={{paddingHorizontal:'2%', marginTop:15, flex: 1, marginLeft: 5}}>
                        //         <Text style={{color:colors.white,fontSize:30, fontWeight:'bold'}} numberOfLines={1}>{items.item.lastName}</Text> 
                        //     </View>
                        // </View>
                        <View style={{paddingHorizontal:'2%', marginTop:15, width: '100%'}}>
                            <Text style={{color:colors.white,fontSize:30, fontWeight:'bold'}} numberOfLines={1}>{items.item.firstName} {items.item.lastName}</Text> 
                        </View>      
                    :
                    null
                    }  
                    ...
                    <View style={{flexDirection:'row',justifyContent: 'flex-start',
                        ...
                        <View style={{flexDirection:'row',marginRight: isAddress || isContect ? '5%':'10%',paddingBottom: isSaleData ? 10 : null, flex: 1, justifyContent: isAddress ? 'center' : 'flex-start', alignItems: 'center',marginLeft:10}}>
                            <View style={{paddingLeft: isAddress || isContect ?'1%' : '20%',alignSelf: 'center', marginRight: isAddress || isContect ?'5%' : '10%'}}>
                                {isSaleData ?
                                    <Icon type="Ionicons" name="podium-outline" style={{color:colors.white,fontSize:30}}/>
                                :
                                    <Icon type="SimpleLineIcons" name="phone" style={{color:colors.white,fontSize:20}}/>
                                }
                            </View>
                        ...
                        <View style={{flexDirection:'row',marginRight:'5%', flex: 1, justifyContent: isAddress ? 'center' : 'flex-start', alignItems: 'center'}}>
                            <View style={{paddingLeft:'1%',alignSelf:'center', marginRight: isAddress || isContect ?'5%' : '5%'}}>
                                {isAddress || isContect ?
                                    <Icon type="SimpleLineIcons" name="printer" style={{color:colors.white,fontSize:20}}/>
                                :
                                null
                                }    
                            </View>
                        ...
                        :
                            <View style={{flexDirection:'row', marginRight:'5%', flex: 1, alignItems: 'center'}}>
                                <View style={{paddingLeft:'1%',alignSelf:'center', marginRight: '5%'}}>
                                    <Icon type="SimpleLineIcons" name="envelope" style={{alignItems:'center',color:colors.white,fontSize:18}}/>
                                </View>
                                <View>
                                    {items.item.email ?
                                ...
                                <View style={{flexDirection:'row',justifyContent:'space-between',marginHorizontal:'5%'}}>
                                    ...
                                    <View style={{marginRight:25}}>
                                        ...
                                        {items.item.officeNameTh ?
                                            <Text style={{color:colors.white,fontSize:22, fontWeight:'bold'}}>{items.item.officeNameTh}</Text>  
                                        :
                                            <Text style={{color:colors.white,fontSize:22, fontWeight:'bold'}}> - </Text>  
                                        }           
                                    </View>  
                                    <View>
                                        ...
                                        {items.item.groupNameTh ?
                                            <Text style={{color:colors.white,fontSize:22, fontWeight:'bold'}}>{items.item.groupNameTh}</Text>  
                                        :
                                            <Text style={{color:colors.white,fontSize:22, fontWeight:'bold'}}> - </Text>  
                                        }           
                                    </View>      
